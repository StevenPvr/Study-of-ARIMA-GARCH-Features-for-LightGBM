...
def test_var_summary_structure() -> None:
    ...
    assert summary["distribution"]["name"] == "normal"  # type: ignore[index]
    levels = summary["levels"]  # type: ignore[assignment]
    assert levels["0.01"]["violations"] == pytest.approx(1.0)  # type: ignore[index]
