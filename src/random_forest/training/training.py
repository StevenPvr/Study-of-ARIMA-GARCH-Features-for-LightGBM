...
from src.constants import (
    ...
    RF_DATASET_SIGMA_PLUS_BASE_FILE as RF_DATASET_SIGMA_PLUS_BASE,
...
def _prepare_training_tasks(
    ...
    # Optional task: sigma-plus-base if present in optimization results
    if "rf_dataset_sigma_plus_base" in opt_results:
        tasks.append(
            (
                RF_DATASET_SIGMA_PLUS_BASE,
                "rf_sigma_plus_base",
                opt_results["rf_dataset_sigma_plus_base"]["best_params"],
            )
        )
...
def _save_training_results(
    ...
    if "rf_sigma_plus_base" in results_dict:
        training_results["rf_sigma_plus_base"] = results_dict["rf_sigma_plus_base"]
...
def run_training(
    ...
    if "rf_sigma_plus_base" in results_dict:
        out["rf_sigma_plus_base"] = results_dict["rf_sigma_plus_base"]
    if "rf_rsi14_only" in results_dict:
