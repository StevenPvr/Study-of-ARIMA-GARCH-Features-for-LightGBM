...
from src.garch.structure_garch.utils import compute_acf
...
def _plot_acf_squared(ax: Any, acf_sq: np.ndarray, conf: float, acf_lags: int) -> None:
    ...
    ax.axhline(
        ...
        linestyle="--",
        linewidth=1.5,
        label=f"Niveau de confiance (±{conf:.3f})"
    )
    ax.axhline(
        -conf,
        color="red",
    ...
    ax.set_title("ACF des résidus ARIMA au carré", fontsize=_FONTSIZE_TITLE, fontweight="bold")
    ax.set_xlabel("Lag", fontsize=12)
    ax.set_ylabel("ACF(ε²)", fontsize=12)
    ax.legend()
    ax.grid(True, alpha=0.3)
...
def _compute_lm_statistic(e2: np.ndarray, lags: int, n: int) -> float:
    ...
    for lag in range(1, lags + 1):
        X[:, lag] = e2[lags - lag : n - lag]

    # Dependent variable (squared residuals without first lags observations)
    y = e2[lags:]

    # OLS regression
    try:
        beta = np.linalg.lstsq(X, y, rcond=None)[0]
        residuals_reg = y - X @ beta
        r_squared = 1 - np.sum(residuals_reg**2) / np.sum((y - np.mean(y))**2)
        return float(n * r_squared)
