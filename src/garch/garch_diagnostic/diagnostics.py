...
def _save_acf_pacf_plots(
    ...
    *,
    ...
    if squared:
        ...
        acf_title = "ACF of standardized squared residuals (z^2)"
        pacf_title = "PACF of standardized squared residuals (z^2)"
        suptitle = "ACF/PACF of standardized squared residuals"
        log_message = "Saved ACF/PACF(z^2) plots"
    else:
        y = z - np.mean(z)
        plot_name = "garch_std_acf_pacf"
        acf_title = "ACF of standardized residuals (z)"
        pacf_title = "PACF of standardized residuals (z)"
        suptitle = "ACF/PACF of standardized residuals"
        log_message = "Saved ACF/PACF(z) plots"
...
def save_histogram_std_residuals(
    ...
    *,
    ...
    """Save histogram of standardized residuals with normal distribution overlay.

    Graphical diagnostic to verify distribution adequacy:
    - Histogram shows empirical distribution of standardized residuals
    - Normal overlay shows expected distribution if residuals are N(0,1)
    - For Student-t distribution, overlay shows t-distribution
    ...
    try:
        fig, canvas, ax = create_figure_canvas(
            figsize=_DIAGNOSTIC_FIGURE_SIZE, n_rows=1, n_cols=1
        )

        # Plot histogram with normal overlay
        # Note: overlay uses N(0,1) as approximation even for Student-t
        ...
        ax.set_title("Distribution of standardized residuals (with N(0,1) overlay)")
