...
def _align_and_compute_residuals(
    ...
    Returns:
        DataFrame with date and arima_resid columns.
    """
...
def _extract_in_sample_residuals(
    ...
    try:
        ...
        if train_df is None:
            return pd.DataFrame({"date": [], "arima_resid": []})
...
def _compute_train_residuals(
    ...
    Args:
        split_df: Split dataset DataFrame.
        fitted_model: Fitted ARIMA model (fallback).
        value_col: Column name for target values.
...
def _get_test_residuals(
    ...
    Returns:
        DataFrame with date and arima_resid columns, or None if unavailable.
    """
    ...
    if "arima_resid" in test_resid_df.columns:
        test_resid_df = pd.DataFrame(test_resid_df[["date", "arima_resid"]].copy())
...
def save_garch_dataset(
    ...
    # If backtest_residuals contains full series residuals (train+test), use them directly
    if backtest_residuals is not None and not backtest_residuals.empty:
        # Use full series residuals directly (train+test combined)
        all_resid_df = backtest_residuals.copy()
        all_resid_df["date"] = format_dates_to_string(all_resid_df["date"])
        all_resid_df = pd.DataFrame(all_resid_df[["date", "arima_resid"]].copy())
        logger.info(
            f"Using full series residuals (train+test) for GARCH: {len(all_resid_df)} observations"
        )
    else:
        # Fallback: separate train and test residuals (legacy behavior)
...
def regenerate_garch_dataset_from_rolling_predictions() -> Path:
    ...
    It also attempts to load the trained ARIMA model to compute training residuals.
    ...
    # Load the trained ARIMA model for computing training residuals
    from src.arima.training_arima.training_arima import load_trained_model

    fitted_model, _ = load_trained_model()
    logger.info("Loaded trained ARIMA model for computing training residuals")
