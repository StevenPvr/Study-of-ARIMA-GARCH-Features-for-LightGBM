...
_EVAL_FIGURE_SIZE = (10, 4)
_EVAL_DPI = 150
_TEXT_POSITION_X = 0.02
_TEXT_POSITION_Y = 0.95
    ...
    if isinstance(y.index, pd.DatetimeIndex):
        ...
        try:
            if len(dt_index) > 1:
                ...
                y_copy.index = pd.date_range(start=dt_index[0], periods=len(dt_index), freq=freq)
                return y_copy
        except Exception:
            pass

        # Last resort: use daily frequency
        y_copy.index = pd.date_range(start=dt_index[0], periods=len(dt_index), freq="D")
        return y_copy
...
def plot_residuals_acf_with_ljungbox(
    ...
    fig = plt.figure(figsize=_EVAL_FIGURE_SIZE)
    ...
    ax.text(_TEXT_POSITION_X, _TEXT_POSITION_Y, txt, transform=ax.transAxes, va="top")

    fig.tight_layout()
    fig.savefig(out_path, dpi=_EVAL_DPI)
    plt.close(fig)
...
def plot_predictions_vs_actual(
    ...
    fig, ax = plt.subplots(figsize=(12, 8), dpi=_EVAL_DPI)
...
"""
    ...
    plt.tight_layout()
    fig.savefig(out_path, dpi=_EVAL_DPI, bbox_inches="tight")
    plt.close(fig)
...
def _setup_backtest_configuration(
    ...
    order: tuple[int, int, int],
    trend: str,
) -> tuple[pd.Series, list[Any], ArimaSpec, int]:
...
def backtest_full_series(
    ...
    Returns:
        DataFrame with columns: date, y_true, y_pred, arima_resid for all dates.
    """
